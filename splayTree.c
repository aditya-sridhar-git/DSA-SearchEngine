...
ChatNode *insert_chat(SplayTree *tree, const char *id, const char *title,
                      long timestamp) {
...
void output_chat(ChatNode *node) {
  ...
  printf("{\"success\":true,\"chat\":{\"id\":\"%s\",\"title\":\"%s\","
         "\"timestamp\":%ld}}",
...
int main(int argc, char *argv[]) {
  if (argc < 2) {
    printf(
        "{\"success\":false,\"error\":\"Usage: splayTree <command> [args]\"}");
    return 1;
  }
